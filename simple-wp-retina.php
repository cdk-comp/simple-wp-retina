...
namespace simplewpretina;

/**
 * SimpleWPRetina class.
 */
class SimpleWpRetina
{
    ...
    {
        add_action('after_setup_theme', array($this, 'afterSetupTheme'), 999);
        add_action('wp_footer', array($this, 'wpFooter'));
        add_filter('the_content', array($this, 'theContent'));
        add_filter('post_thumbnail_size', array($this, 'postThumbnailSize'), 999, 1);
        add_filter('post_thumbnail_html', array($this, 'postThumbnailHtml'), 999, 5);

        add_filter('post_gallery', array($this, 'postGallery'), 999, 2);
    }
    ...
    public function afterSetupTheme()
    {
        ...
        foreach ($image_sizes as $image_size) {
            ...
            if ($image = $this->getImageSize($image_size)) {
                add_image_size($image_size . '@2x', $image['width'] * 2, $image['height'] * 2, $image['crop']);
            } else {
                if ($image_size == 'thumb' || $image_size == 'thumbnail') {
                    add_image_size(
                        'thumbnail@2x',
                        intval(get_option('thumbnail_size_w')) * 2,
                        intval(get_option('thumbnail_size_h')) * 2,
                        true
                    );
                } else {
                    if ($image_size == 'medium') {
                        add_image_size(
                            'medium@2x',
                            intval(get_option('medium_size_w')) * 2,
                            intval(get_option('medium_size_h')) * 2,
                            false
                        );
                    } else {
                        if ($image_size == 'large') {
                            add_image_size(
                                'large@2x',
                                intval(get_option('large_size_w')) * 2,
                                intval(get_option('large_size_h')) * 2,
                                false
                            );
                        }
                    }
    ...
    public function postThumbnailSize($size)
    {
        if ($this->isHighRes() && $size != 'full') {
            $size .= '@2x';
        }
        return $size;
    }
    ...
    public function postThumbnailHtml($html, $post_id, $post_thumbnail_id, $size, $attr)
    {
        if ($this->isHighRes()) {
            ...
            foreach ($this->breakApartImages($matches[1]) as $img_key => $image) {
    ...
    /**
     * breakApartImages function.
     *
     * @param $images
     * @return array
     */
    public function breakApartImages($images)
    {
    ...
    public function theContent($content)
    {
        ...
        if ($this->isHighRes()) {
            ...
            foreach ($this->breakApartImages($matches[1]) as $key => $image) {
                if (!isset($image['class'])) {
                    $image['class'] = 'not_determined';
                }
...
$simple_wp_retina = new SimpleWpRetina();
